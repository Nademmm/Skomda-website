...
1. `next lint` dan `tsc --noEmit` — 0 error.
2. Cek semua <a> dan <button> punya handler/href valid (grep untuk href="#" atau onClick kosong yang mencurigakan).
3. Cek tiap <img>/<Image> punya prop alt yang tidak kosong.
4. Cek heading hierarchy per halaman (h1 tunggal, h2/h3 berurutan, tidak skip level).
5. Kalau ada perubahan halaman publik, jalankan build lokal (`npm run build`) dan laporkan warning terkait metadata/SEO.
6. Bandingkan desain baru dengan prinsip di frontend-design skill — flag kalau terasa generik/template.
